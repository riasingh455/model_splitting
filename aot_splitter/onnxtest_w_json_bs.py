...
def silly_test(path, rank, iters, f_ts):
    # input_name = session.get_inputs()[0].name
    # print(shape_dict[rank])
    ort_sess = ort.InferenceSession(f'{path}/split_{rank}_quant.onnx', sess_options=sess_opt, providers=providers)
    

    # sess_opt.execution_mode = ort.ExecutionMode.ORT_SEQUENTIAL
    # sess_opt.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    x=[np.empty(tuple(numpy_shape), dtype=np.float32)]
...
if __name__=="__main__":
    ...
    sess_opt.intra_op_num_threads = cores
    ...
    silly_test(path, rank, iters, f_ts)
    # for core in range(cores):
    #     p = mp.Process(target=silly_test, args=( core, path, rank, iters, f_ts, ))
    #     p.start()
    #     procs.append(p)
