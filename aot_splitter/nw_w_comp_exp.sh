...
do 
    ...
    do 
        ...
        for (( n=0;n<${#nodes[@]};n++ ))
        do
            command="python3 onnx_w_zmq.py 1,10 ./${model_type}_${model_split}_${world}_1 ${n},${world} 4 9123 ${nodes_string}"
            full_command="pushd $path_prefix/aot_splitter; source /home/animesh/model_splitting/pi-torch/bin/activate; ${command} > ${path_dst}/speed_chronos${nodes[$n]}_${world}_${rank}.log &"
        ...
        for (( n=0;n<${#nodes[@]};n++ ))
        do
            command="python3 onnx_w_zmq.py 5,10 ./${model_type}_${model_split}_${world}_1 ${n},${world} 4 9123 ${nodes_string}"
            full_command="pushd $path_prefix/aot_splitter; source /home/animesh/model_splitting/pi-torch/bin/activate; ${command} > ${path_dst}/speed_chronos${nodes[$n]}_${world}_${rank}.log &"
            timeout 10m ssh -o ConnectTimeout=10 -o BatchMode=yes -o StrictHostKeyChecking=no animesh@${nodes[$n]} "${full_command}" &
            waiters+=($!)
