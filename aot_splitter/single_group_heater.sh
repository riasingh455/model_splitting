    ...
    do
        ...
        do
            ...
            do
                ...
                path_dst="${path_prefix}/logs/single_group_heat/${node_prefix}/${model_type}_${model_split}/${conc_val}_concurrency_level/${repeat}/${fake_world}_${cores}/"
                mkdir -p ${path_dst}
                waiters=()
                for (( n=0;n<${#nodes[@]};n++ ))
                do
                    ...
                    command="python3 single_runner.py --cores ${cores} --rank ${n} --world ${conc_val} --ip ${nodes[0]} --port 8123 --warmup 1 --batch-size 1 --batch-num 1 --iters 10 --model-type ${model_type} --model-split-type ${model_split}"
