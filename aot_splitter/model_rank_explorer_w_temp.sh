...
do
    ...
    do
        ...
        do
                ...
                path_dst="${path_prefix}/logs/single_group_heat_con/${node_prefix}/${model_type}_${model_split}_onnx/${repeat}/"
                mkdir -p ${path_dst}
                waiters=()
                killers=()
                for (( n=0;n<${#nodes[@]};n++ ))
                do
                    ...
                    command="python3 onnxtest.py ./${model_type}_${model_split}_${fake_world}_1_custom 20 ${fake_rank} ${cores} ${dt}"
                    # if [[ $wait_flag -eq 1 ]]
                    # then
                ...
                for (( n=0;n<${#nodes[@]};n++ ))
                do
                    ssh -o ConnectTimeout=10 -o BatchMode=yes -o StrictHostKeyChecking=no animesh@${nodes[$n]} "kill -9 ${killers[$n]}; pkill -9 temp_speed; pkill -9 model_rank; pkill -9 python3;"
