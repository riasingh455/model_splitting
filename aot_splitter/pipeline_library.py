...
class GenModel:
    ...
    def split(self, example_input, rank:int, split_num:int = 2, input_count:int =1,  model_split_type="children", specific_chunks:List[int] = []):
        ...
        for s in range(split_num):
            # temp = pipe.build_stage(s, "cpu")#pipe.get_stage_module(s)
            temp = pipe.get_stage_module(s)#.run_decompositions(decomp_table=None)
            ...
            if s == 0:
                ...
                exp = torch.export.export(temp, tuple({example_input}) )
                torch.export.save(exp, f"test{s}.pt2")
                # output = temp.stage_module.forward(example_input)
            else:
                if type(output)!=type(example_input):
                    ...
                    # torch.export.save(exp, f"test{s}.pt2")
                    # output = temp.stage_module.forward(*output)
                else:
                    output = temp.forward(output)
                    # output = temp.stage_module.forward(output)
        ...
        for inp in range(input_count):
            if split_num==1:
                # mod = pipe.get_stage_module(rank)
                # exp = torch.export.export(mod)
                # torch.export.save(exp, f"test{rank}.pt2")
                # torch.export.save(mod, f"test{rank}")
                # f=open(f"test{rank}.py", "w")
                # f.write(mod)
                # print(mod)
                # mod.compile()
                # mod.share_memory()
                # name = pipe.get_stage_module(rank)._get_name()
                # print(name)
