...
def custom_pipeline(aot_dir, batch_num, world, rank, cores, iters=1, w_iters=1, inputs=None):
    ...
    split_pt = f"{aot_dir}/{rank}.model"
    # split_pt = f"{aot_dir}/exe_split_{rank}.pte.exp"
    # mod = export.load(split_pt).module()
    # mod = torch.load(split_pt)
    if world==2:
        if rank==0:
            from resnet18_children_2_1.test_folder_0 import test_0 as model
        if rank == 1:
            from resnet18_children_2_1.test_folder_1 import test_1 as model
...
def data_loader(model_type, batch_num, batch_size, image_path):
    ...
    img_batch=[torch.randn(3, 224, 224)]*batch_size
    # print(img_batch)
