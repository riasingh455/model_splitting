...
for repeat in 1
do
    for fake_world in 1 5
    do
        for conc_val in 1 10
        do
            
            nodes=( $(node_select) )
            fake_rank_idx=$(( $fake_world-1 ))
            fake_rank=${highest_flop_rank[$fake_rank_idx]}
            path_dst="${path_prefix}/logs/single_heat/${node_prefix}/${model_type}_${model_split}/${conc_val}_concurrency_level/${repeat}/${fake_world}_${fake_rank}/"
            mkdir -p ${path_dst}
            waiters=()
            for (( n=0;n<${#nodes[@]};n++ ))
            do
			    ...
			    command="python3 single_runner.py --cores 4 --rank ${n} --world ${conc_val} --ip ${nodes[0]} --port 8123 --warmup 1 --batch-size 1 --batch-num ${bnum} --iters 10 --model-type ${model_type} --model-split-type ${model_split}"
                command="${command} --fake_rank ${fake_rank} --fake_world ${fake_world}"
                timeout 10m ssh -o ConnectTimeout=10 -o BatchMode=yes -o StrictHostKeyChecking=no animesh@${nodes[$n]} "pushd $path_prefix; ${command} > ${path_dst}/speed_chronos${nodes[$n]} &" &
                waiters+=($!)
            done
