...
do
	...
	do
    		...
    		for n in ${nodes[@]}
    		do
         		node_prefix=$node_prefix world=$world rank=$counter master=${nodes[0]} model_type=$model_type model_split=$model_split srun --nodelist=$n roofline_pi_script_no_temp_reader.sh &
	 		counter=$(( $counter+1 ))
			#echo $n, $world
    		done
