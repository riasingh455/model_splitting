...
do
	...
	do
		...
		waiters=()
			...
			ssh -o ConnectTimeout=10 -o BatchMode=yes -o StrictHostKeyChecking=no animesh@${nodes[$n]} "pushd $path_prefix; node_prefix=$node_prefix world=$world rank=$counter master=${nodes[$master_idx]} model_type=$model_type model_split=$model_split ./roofline_pi_script_no_slurm_no_temp.sh &" &
			waiters+=($!)
			counter=$(( $counter+1 ))
			#echo $n, $world
    		done
