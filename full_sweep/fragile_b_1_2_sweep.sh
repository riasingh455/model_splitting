...
log_path="/home/animesh/full_sweep_logs/fragile_rank/ind/b-${1}-${2}/"
...
specific_fragile_test () {
 ...
 do
	 if [ $(sinfo -N | grep "${val}-${f}" | grep "idle" | grep -v "idle\*" | wc -l) -eq 0 ]
         then
               continue
	 fi
	 visited+=("${val}-${f}")
 ...
 for v in ${visited[@]}
 do
     log_path=$log_path start=4 end=4 size=$1 srun -N 1 --nodelist="${v}" ${script_path}/volt_tester.sh 4 0  > /dev/null&
     pids+=($!)
 done
