...
do
	...
	do
			...
			for(( n=0; n<${#nodes[@]}; n++ ))
				...
				then
					...
					counter=0
         			...
         			node_prefix=$node_prefix world=$world rank=$counter master=${nodes[$master_idx]} model_type=$model_type model_split=$model_split srun --nodelist=${nodes[$n]} roofline_pi_script_conc.sh &
				pids+=($!)
				#echo $n, $world
	 			counter=$(( $counter+1 ))
    			done
		...
		#done
		wait "${pids[@]}"
                mkdir -p ${path_prefix}/logs/roofline/${node_prefix}/${model_type}_${model_split}/${repeat}/con_${conc_val}/
                mv ${path_prefix}/logs/roofline/${node_prefix}/${model_type}_${model_split}/${world}_size ${path_prefix}/logs/roofline/${node_prefix}/${model_type}_${model_split}/${repeat}/con_${conc_val}/
    		#break
	done
done
