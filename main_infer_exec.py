...
class Args:
    ...
    device: str = "cpu"
    image: str = "./bear.jpeg"
    images: List[str] = [] #if multiple images provided, each image run an iters+warmups number of times 
...
if __name__ == "__main__":
    ...
    for _ in range(l):
        if args.assigned_rank==0:
            x0 = pipe_mod.load_image_tensor(args.image, pre_proc, device) 
        output = pipe_mod.pipeline_inference(args.backend, args.stages, args.assigned_rank, args.warmup, args.iters)
        if output!=None:
