...
# install
# pip install "transformers[torch]==4.57.6" "huggingface_hub<1.0" accelerate

# adapted the pipeline-based image model code to support TimesFM by wrapping its inputs, changing the input shape, 
# and bypassing pipeline splitting since transformers can’t be traced by torch.pipeline.
...
class TimesFMWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        freq = torch.zeros(x.shape[0], dtype=torch.long, device=x.device)
...
@torch.no_grad()
def stat_generator_timesfm(model, model_type, model_split_type, world, batch_size):
    """
    TimesFM does not work with torch.distributed.pipelining.pipeline tracing.
    So for TimesFM, we run it as one stage and still write flop.dict + stages.dict.
    """
    example_input = torch.randn(batch_size, 512)
    ...
    stage_shapes[0] = [tuple(example_input.shape), f"{example_input.dtype}"]

    flop_counter = FlopCounterMode(display=False, depth=None)
    with flop_counter:
        output = model(example_input)

    flop_stages[0] = flop_counter.get_total_flops()
    stage_shapes[1] = [tuple(output.shape), f"{output.dtype}"]
    ...
    print("TimesFM ran successfully without pipeline tracing.")
    print("Output shape:", output.shape)
    print("FLOPs:", flop_stages)
...
def model_splitter(model, model_type, split_type, world, batch_size, specific_chunks=[]):
    # UPDATED: TimesFM cannot go through pipeline(), so handle it separately.
    if model_type == "timesfm":
        stat_generator_timesfm(model, model_type, split_type, world, batch_size)
        return
...
if __name__ == "__main__":
    ...
    if args.model_type == "mbv3_small":
        ...
        pretrained_model = TimesFMWrapper().eval()
    ...
    model_splitter(
        ...
        args.world,
        args.batch_size,
        args.custom if args.custom != None else []
    )
