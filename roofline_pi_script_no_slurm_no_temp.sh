...
killer () {
        kill -9 $(ps -aex | grep "main_runner.py" | awk '{print $1}')
        kill -9 $(ps -aex | grep "python3" | awk '{print $1}')
        #kill -9 $(ps -aex | grep "temp_speed_reader.sh" | awk '{print $1}')
        #kill -9 $(ps -aex | grep "roofline" | awk '{print $1}')
	#if [[ $(ps -aex | grep "roofline_pi_script" | wc -l) -gt 2 ]]
	#then
		#echo "inside but why?"
	#	for p in $(ps -aex | grep "roofline_pi_script" | awk '{print $1}')
	#	do
	#		if [[ $(ps -aex | grep "roofline_pi_script" | wc -l) -eq 2 ]]
	#		then
	#			break
	#		fi
	#		kill -9 $p
	#	done
	#fi
	echo 'race4fun' | sudo -S sh -c 'sync; echo 3 > /proc/sys/vm/drop_caches';
}
