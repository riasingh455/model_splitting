...
def executorch_stuff(fname, exp, cal_sample, ex_inp, star=False):
    ...
    torch.export.save(re_exp, f"{fname}.pt2")#"resnet_exp_strict_rexp.model")
...
# quantize_(m, Int4WeightOnlyConfig(group_size=32, version=1))
            ...
            # temp = executorch_stuff(f"{app_dir}/exe_split_{rank}.pte", exp.module(), example_input, (example_input,)) CHANGE
            temp = executorch_stuff(f"{app_dir}/exe_split_{rank}", exp.module(), example_input, (example_input,))
            ...
            if type(output)!=type(example_input):
                ...
                #temp = executorch_stuff(f"{app_dir}/exe_split_{rank}.pte", exp.module(), output, output, star=True) CHANGE
                temp = executorch_stuff(f"{app_dir}/exe_split_{rank}", exp.module(), output, output, star=True)
                # torch.export.save(exp, f"{app_dir}/split_{rank}.pt2")
                n_output = temp.forward(*output)
                
            else:
                exp = torch.export.export(temp, (output,))
                #temp = executorch_stuff(f"{app_dir}/exe_split_{rank}.pte", exp.module(), output, (output,)) CHANGE
                temp = executorch_stuff(f"{app_dir}/exe_split_{rank}", exp.module(), output, (output,))
                # torch.export.save(exp, f"{app_dir}/split_{rank}.pt2")
                n_output = temp.forward(output)
...
if __name__ == "__main__":
    ...
    if args.model_type=="mbv3_small":
        ...
        pretrained_model = resnet50(weights=weights).eval()
    elif args.model_type == "resnet101":
        weights = ResNet101_Weights.DEFAULT
        pretrained_model = resnet101(weights=weights).eval()
